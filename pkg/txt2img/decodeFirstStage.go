...
func computeFirstStage(workCtx *ggml.Context, x *ggml.Tensor, decode bool) *ggml.Tensor {
	...
	if decode {
		...
		result = ggml.NewTensor4d(workCtx, ggml.TYPE_F32, W/8, H/8, useSmallAutoencoder() ? 4 : 8, x.Ne[3])
	}
	t0 := ggml.TimeMs()
	if !useSmallAutoencoder() {
		if decode {
...
func encodeFirstStage(workCtx *ggml.Context, x *ggml.Tensor) *ggml.Tensor {
	return computeFirstStage(workCtx, x, false)
}

func decodeFirstStage(workCtx *ggml.Context, x *ggml.Tensor) *ggml.Tensor {
	return computeFirstStage(workCtx, x, true)
}
